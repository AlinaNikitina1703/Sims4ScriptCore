...
from scripts_core.sc_util import error_trap, init_sim, clean_string, message_box, error_trap_console
...
def go_here_routine(sim, location, level=0, offset=1.5):
    count = 25
    success = None
    pos = location
...
def pause_routine(timeout):
    ...
    if now - sc_Vars.timestamp < date_and_time.create_time_span(minutes=timeout):
        return True
    sc_Vars.timestamp = now
    return False
...
def debugger(debug_text, frame=1, full_frame=False, write=False, console_only=False, sim_time=True):
    try:
        ...
        if full_frame and not frame and not console_only:
        ...
        elif full_frame and frame and not console_only:
            for f in range(1, frame+1, 1):
                ...
                line_number = frameinfo.f_lineno
                debug_text = debug_text + "\n@{} - {} - {}".format(line_number, filename, func_name)
        elif frame and not console_only:
            debug_text = debug_text + "\n@{} - {} - {}".format(line_number, filename, func_name)
        if write and not console_only:
            datapath = os.path.abspath(os.path.dirname(__file__))
